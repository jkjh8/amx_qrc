...
def get_data_from_server():
    global main_server_ip_addr, page, local_device, venue_name, zone_name
    REQ_PATH = "/api/amx"
    ...
    if "barixes" in data:
        for id, ip in enumerate(data["barixes"]):
            if ip:
                page["barixes_ip_addr"][id + 1] = ip
        logger.info('barixes_ip_addr: %s' % page["barixes_ip_addr"])
    ...
    if "qsys" in data:
        ...
        if "ZoneStatus" in local_device:
            zones = []
            for idx, zone in enumerate(local_device["ZoneStatus"]):
                if zone["name"]:
                    zones.append(zone["name"])
                else:
                    zones.append(f"지역-{idx + 1}")
            ...
            if zones != zone_name:
                ...
                page["tp_qrc_on_air_zone_list"] = adjust_list_length(page["tp_qrc_on_air_zone_list"], len(zone_name), False)
                page["qrc_zone_gain_status"] = adjust_list_length(page["qrc_zone_gain_status"], len(zone_name), 0.0)
                page["qrc_zone_mute_status"] = adjust_list_length(page["qrc_zone_mute_status"], len(zone_name), False)
                page["qrc_zone_on_air_status"] = adjust_list_length(page["qrc_zone_on_air_status"], len(zone_name), False)
...
def get_barixes():
    ...
    for id, ip in enumerate(json.loads(response_data)):
        if ip is not None:
            page["barixes_ip_addr"].update({id + 1: ip})
...
def barix_set_relay_all(state):
    global logger, page
    try:
        for ip in page["barixes_ip_addr"].values():
...
def barix_set_relays (zone_idx_list, state):
    ...
    try:
        for idx in zone_idx_list:
            if idx in page["barixes_ip_addr"]:
                barix_set_relay(page["barixes_ip_addr"][idx], state)
    except Exception as e:
